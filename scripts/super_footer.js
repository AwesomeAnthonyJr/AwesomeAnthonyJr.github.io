//JAVASCRIPT FILE FOR THE SUPER FOOTER STUFF
const superFooter = document.querySelector('.super-footer');

superFooter.innerHTML = `
<div class ="super-footer-colors">
    <div class ="footer-g"></div>
    <div class ="footer-y"></div>
    <div class ="footer-c"></div>
    <div class ="footer-m"></div>
    <div class ="footer-o"></div>
    <div class ="footer-b"></div>
    <div class ="footer-r"></div>
</div>
<div class="super-footer-grid">
    <ul class="super-footer-nav">
        <li><a href="/">HOME</a></li>
        <li><a href="/about/">ABOUT</a></li>
        <li><a href="/games/">GAMES</a></li>
        <li><a href="/journal/">JOURNAL</a></li>
        <li><a href="/">EXTRAS</a></li>
        <li><a href="/">SECRETS</a></li>
        <li><a href="/links/">LINKS</a></li>
    </ul>
    <ul class="super-footer-links">
        <li><a href="https://leaflight-studios.itch.io/">itch.io|LeafLight Studios</a></li>
        <li><a href="https://leaflight-studios.itch.io/agj0project">itch.io|SHAHARAZON</a></li>
        <li><a href="https://leaflight-studios.itch.io/wyrm-canyon">itch.io|Wyrm Canyon</a></li>
        <li><a href="https://leaflight-studios.itch.io/goblin-knight">itch.io|goblin knight</a></li>
        <li><a href="https://leaflight-studios.itch.io/mage-hand">itch.io|Mage Hand</a></li>
        <li><a href="https://leaflight-studios.itch.io/kill-protocol">itch.io|Kill Protocol</a></li>
        <li><a href="https://leaflight-studios.itch.io/xcvb">itch.io|XCVB</a></li>
    </ul>
</div>
<div class ="super-footer-bottom">Website by Anthony D. Salsbury</div>
<div class ="super-footer-bottom">© 2025 LeafLight Studios. All rights reserved.</div>
`;